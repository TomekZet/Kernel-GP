...
Algorytm został napisany w języku Java z użyciem bibliotek \emph{ECJ (Evolutionary Computing in Java)} \cite{sean_ecj_2010} oraz \emph{LibSVM} \cite{chang_libsvm:_2011}. Pierwsza z nich dostarcza mechanizmy \textit{obliczeń ewolucyjnych} w tym \textit{programowania genetycznego}. LibSVM to klasyfikator SVM napisany oryginalnie w języku C z dostępną implementacją w Javie.
...
W celu przeprowadzenia eksperymentów obliczeniowych z użyciem różnych kombinacji parametrów został napisany skrypt w języku Python. Jego zadaniem było iteracyjne ustawianie kolejnych wartości parametrów, wywołanie właściwego algorytmu z użyciem tych parametrów oraz zapis zwróconych przez algorytm wyników do pliku.
